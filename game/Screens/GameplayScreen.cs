...
public class GameplayScreen : Screen
{
    ...
    public GameplayScreen(CrewbossGame game, string mapName) : base(game)
    {
    ...
    public override void LoadContent()
    {
        ...
        try
        {
            ...
            string blockName = _mapName;
            string generatedDir = Path.Combine(Tweaks.ContentRoot(), "Maps", blockName);
            ...
            {
                throw new FileNotFoundException($"Map '{blockName}' not found at {blockPng} — run dev/regen.sh");
            }
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (Pressed(Keys.F5))
        {
            ...
            string projectDir = repo == null ? null : System.IO.Path.Combine(repo, "platforms", "Desktop");
            bool spawned = false;
            if (projectDir != null && System.IO.File.Exists(System.IO.Path.Combine(projectDir, "Crewboss.Desktop.csproj")))
            {
                try
                {
            ...
            Tweaks.Load();
            var fresh = new GameplayScreen((CrewbossGame)Game, _mapName);
