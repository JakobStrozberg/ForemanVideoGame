...
public class MainMenuScreen : Screen
{
    ...
    private void UpdateMapSelection(GameTime gameTime)
    {
        ...
        if (!_isButtonAnimating && IsMouseClicked())
        {
            ...
            if (_map1ButtonRect.Contains(mousePosition))
            {
                ...
                GameplayScreen gameplayScreen = new GameplayScreen(Game, "Block1");
            ...
            else if (_map2ButtonRect.Contains(mousePosition))
            {
                ...
                GameplayScreen gameplayScreen = new GameplayScreen(Game, "Block1"); // TODO: Block2 once it exists
                // Register the screen with the screen manager
                ScreenManager screenManager = GetScreenManager();
                if (screenManager != null)
                {
    ...
    // Helper method to get the screen manager from CrewbossGame
    private ScreenManager GetScreenManager()
    {
        // Access the screen manager directly using the property in CrewbossGame
        return (Game as CrewbossGame)?.ScreenManager;
    }
} 